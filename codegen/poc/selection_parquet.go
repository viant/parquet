...
func Fields(compression compression) []Field {
	var opt sch.ConvertedType = sch.ConvertedType_TIMESTAMP_MILLIS
	var optStrCType sch.ConvertedType = sch.ConvertedType_UTF8
	var optStringType = parquet.OptionalLogicalType(&sch.LogicalType{
		STRING: &sch.StringType{},
	})
	return []Field{
		NewInt64OptionalField(readTimestamp, writeTimestamp, []string{"timestamp"}, []int{1}, optionalFieldCompression(compression),parquet.OptionalLogicalType(&sch.LogicalType{
		...
		}),parquet.OptionalConvertedType(&opt)),
		NewStringOptionalField(readSID, writeSID, []string{"sid"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
		NewStringOptionalField(readAuctionID, writeAuctionID, []string{"auctionId"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType),parquet.OptionalConvertedType(&optStrCType)),
		NewStringOptionalField(readHostNodeIP, writeHostNodeIP, []string{"hostNodeIp"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
		NewStringOptionalField(readHostNodeVersion, writeHostNodeVersion, []string{"hostNodeVersion"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
		NewStringOptionalField(readEndpoint, writeEndpoint, []string{"endpoint"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
		NewInt64OptionalField(readTimeTakenMcs, writeTimeTakenMcs, []string{"timeTakenMcs"}, []int{1}, optionalFieldCompression(compression)),
		NewStringOptionalField(readIP, writeIP, []string{"ip"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
		...
		NewStringOptionalField(readError, writeError, []string{"error"}, []int{1}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
		NewStringOptionalField(readTimeouts, writeTimeouts, []string{"timeouts"}, []int{2}, optionalFieldCompression(compression),optStringType,parquet.OptionalConvertedType(&optStrCType)),
...
func readTimestamp(x Selection) ([]int64, []uint8, []uint8) {
	switch {
	case x.Timestamp == nil:
		return nil, []uint8{0}, nil
	default:
		return []int64{*x.Timestamp}, []uint8{1}, nil
	}
}

func writeTimestamp(x *Selection, vals []int64, defs, reps []uint8) (int, int) {
	def := defs[0]
	switch def {
	case 1:
		x.Timestamp = pint64(vals[0])
		return 1, 1
	}
...
func newParquetWriter(w io.Writer, opts ...func(*ParquetWriter) error) (*ParquetWriter, error) {
	...
	if p.meta == nil {
		...
		for i, f := range ff {
			schema[i] = f.Schema()
			schema[i].LogicalType = f.LogicalType()
			schema[i].ConvertedType = f.ConvertedType()
		}
